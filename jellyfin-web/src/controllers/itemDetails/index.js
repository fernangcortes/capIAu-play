...
// CapIAu: Custom ordering for collections
...
// CapIAu-Streaming: Feature #17 — YouTube Trailer Button
let capiauCurrentItem = null;

function setYouTubeTrailerButtonVisibility(page, item) {
    capiauCurrentItem = item;
    const isVideoContent = ['Movie', 'Series', 'Episode', 'Season'].indexOf(item.Type) !== -1;
    if (isVideoContent) {
        hideAll(page, 'btnYouTubeTrailer', true);
    } else {
        hideAll(page, 'btnYouTubeTrailer');
    }
}

function onYouTubeTrailerClick() {
    const item = capiauCurrentItem;
    if (item) {
        const searchName = item.OriginalTitle || item.Name || '';
        const query = encodeURIComponent('Trailer ' + searchName);
        window.open('https://www.youtube.com/results?search_query=' + query, '_blank');
    }
}

// CapIAu-Streaming: Feature #18 — Report Error Button
function setReportErrorButtonVisibility(page, item) {
    capiauCurrentItem = item;
    const isVideoContent = ['Movie', 'Series', 'Episode', 'Season'].indexOf(item.Type) !== -1;
    if (isVideoContent) {
        hideAll(page, 'btnReportError', true);
    } else {
        hideAll(page, 'btnReportError');
    }
}

function onReportErrorClick() {
    const item = capiauCurrentItem;
    if (!item) return;

    const errorTypes = [
        { name: '🔇 Sem áudio', id: 'no_audio' },
        { name: '🖼️ Sem vídeo / tela preta', id: 'no_video' },
        { name: '🔤 Legenda com problema', id: 'subtitle_issue' },
        { name: '⏱️ Áudio/vídeo dessincronizado', id: 'desync' },
        { name: '💥 Travando / buffering', id: 'buffering' },
        { name: '📁 Arquivo corrompido', id: 'corrupt' },
        { name: '🎬 Filme/episódio errado', id: 'wrong_content' },
        { name: '📝 Outro', id: 'other' }
    ];

    import('../../components/actionSheet/actionSheet').then((actionsheet) => {
        actionsheet.show({
            title: 'Reportar Erro - ' + (item.Name || ''),
            items: errorTypes.map(t => ({ name: t.name, id: t.id })),
            positionTo: document.querySelector('.btnReportError')
        }).then(selectedId => {
            const errorType = errorTypes.find(t => t.id === selectedId);
            if (errorType) {
                // CapIAu: Sending email via FormSubmit API
                const WEBHOOK_URL = 'https://formsubmit.co/ajax/vozesdoasfalto@gmail.com';
                const reportContent = `🐛 Erro Reportado\n\nTipo: ${errorType.name}\nItem: ${item.Name}\nTipo de mídia: ${item.Type}\nID: ${item.Id}\nHora: ${new Date().toLocaleString('pt-BR')}`;
                
                const reportData = {
                    _subject: `⚠️ Relatório de Erro: ${item.Name}`,
                    _template: 'box',
                    mensagem: reportContent
                };
                ...
                console.log('[CapIAu] Error reported to email:', reportData);

                import('../../components/toast/toast').then(toastModule => {
                    const showToast = toastModule.default || toastModule;
                    showToast('Erro reportado com sucesso! Obrigado.');
                });
            }
        }).catch(() => { /* cancelled */ });
    });
...
function reloadFromItem(instance, page, params, item, user) {
    ...
    setYouTubeTrailerButtonVisibility(page, item);
    setReportErrorButtonVisibility(page, item);
...
// --- CapIAu-Streaming Firebase Status Panel ---
async function bindCapIauFirebaseStatus(page, mediaId) {
    let statusPanel = page.querySelector('.capiau-status-panel');

    // Toggle on/off: se o painel já existe, apenas mostra/oculta
    if (statusPanel) {
        statusPanel.classList.toggle('hide');
        return;
    }

    const insertionPoint = page.querySelector('.mainDetailButtons');
    if (!insertionPoint) return;

    insertionPoint.insertAdjacentHTML('afterend', `
        <div class="capiau-status-panel focuscontainer-x" style="margin-top: 15px; display: flex; align-items: center; gap: 15px; padding: 10px; background: rgba(0,0,0,0.4); border-radius: 8px;">
            <div style="font-size: 14px; font-weight: bold; display: flex; flex-direction: column;">
                <span style="color: #aaa; font-size: 11px; text-transform: uppercase;">Aprovação de Corte</span>
                <div class="capiau-status-history" style="font-size: 12px; color: #ccc;">Carregando...</div>
            </div>
            <select class="capiauStatusSelect emby-select-withcolor" style="padding: 8px; border-radius: 6px; background: #222; color: #fff; border: 1px solid #444; width: auto; font-size: 14px; cursor: pointer;">
                <option value="none" style="color:#fff;">--- Status Atual ---</option>
                <option value="v1" style="color:yellow;">🟡 V1 (Rough Cut)</option>
                <option value="v2" style="color:orange;">🟠 V2 (Ajustes Finais)</option>
                <option value="approved" style="color:lightgreen;">🟢 Aprovado (Picture Lock)</option>
                <option value="rejected" style="color:lightcoral;">🔴 Reprovado / Retrabalho</option>
            </select>
        </div>
    `);

    const statusSelect = page.querySelector('.capiauStatusSelect');
    const statusHistory = page.querySelector('.capiau-status-history');
    if (!statusSelect || !statusHistory) return;

    let firebaseEngine;
    try {
        firebaseEngine = await import('../../scripts/capiau/firebaseConfig.js');
    } catch (e) {
        console.error('CapIAu Status Panel: Firebase inacessível.', e);
        return;
    }

    const { db, collection, onSnapshot, query, where, addDoc } = firebaseEngine;

    const q = query(
        collection(db, 'capiau_status'),
        where('mediaId', '==', mediaId)
    );

    onSnapshot(q, (snapshot) => {
        const sortedDocs = snapshot.docs.map(i => i.data()).sort((a, b) => b.timestamp - a.timestamp);
        if (sortedDocs.length > 0) {
            const latest = sortedDocs[0];
            statusSelect.value = latest.status;
            const dt = new Date(latest.timestamp).toLocaleString();
            statusHistory.innerText = `Atualizado por ${latest.user} em ${dt}`;
            if (latest.status === 'approved') statusHistory.style.color = 'lightgreen';
            else if (latest.status === 'rejected') statusHistory.style.color = 'lightcoral';
            else statusHistory.style.color = '#ccc';
        } else {
            statusSelect.value = 'none';
            statusHistory.innerText = 'Nenhuma marcação gravada ainda.';
        }
    });

    statusSelect.addEventListener('change', async function() {
        const novoStatus = this.value;
        if (novoStatus === 'none') return;
        let capiauUserName = localStorage.getItem('capiauUserName');
        if (!capiauUserName) {
            capiauUserName = prompt('Identifique seu cargo ou nome:', 'Membro da Produtora') || 'Produtora';
            localStorage.setItem('capiauUserName', capiauUserName);
        }
        try {
            statusHistory.innerText = 'Sincronizando...';
            await addDoc(collection(db, 'capiau_status'), {
                mediaId: mediaId,
                status: novoStatus,
                user: capiauUserName,
            ...
            console.error('CapIAu Status Erro:', e);
            alert('Não foi possível salvar o status na Nuvem.');
        }
    });
}
// --- Fim CapIAu Status Panel ---
...
// CapIAu-Streaming: Escuta global para o painel mestre ativado pelo menu de 3 pontinhos
window.removeEventListener('capiauToggleStatusPanel', window._capiauTogglePanelHandler);
window._capiauTogglePanelHandler = function(e) {
    const currentPage = document.querySelector('.page.itemDetailPage:not(.hide)');
    if (currentPage && e.detail && e.detail.mediaId) {
        bindCapIauFirebaseStatus(currentPage, e.detail.mediaId);
    }
};
window.addEventListener('capiauToggleStatusPanel', window._capiauTogglePanelHandler);
...
export default function (view, params) {
    ...
    function init() {
        ...
        // CapIAu-Streaming: Feature #17 — YouTube Trailer
        bindAll(view, '.btnYouTubeTrailer', 'click', onYouTubeTrailerClick);
        // CapIAu-Streaming: Feature #18 — Report Error
        bindAll(view, '.btnReportError', 'click', onReportErrorClick);
