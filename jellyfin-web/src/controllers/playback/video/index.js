...
export default function (view) {
    ...
    function updateDisplayItem(itemInfo) {
        ...
        const trickplayResolutions = mediaSourceId ? item.Trickplay?.[mediaSourceId] : null;
    ...
    function updateNowPlayingInfo(player, state) {
        ...
        if (!item) {
            ...
            btnFrameForward.disabled = true;
            btnRewind.disabled = true;
            btnFrameBack.disabled = true;
        ...
        btnFrameForward.disabled = false;
    ...
    function onInputCommand(e) {
        ...
        switch (e.detail.command) {
            ...
            // CapIAu-Streaming: Feature #31 — Frame-by-frame in player
            case 'frameforward':
            case 'frameback':
                // Pause if playing, then the inputManager handles the seek
    ...
    function updatePlayerStateInternal(event, player, state) {
        ...
        btnFrameForward.disabled = !playState.CanSeek;
        btnRewind.disabled = !playState.CanSeek;
        btnFrameBack.disabled = !playState.CanSeek;
    ...
    function updateTimeText(elem, ticks, divider) {
        ...
        // CapIAu-Streaming: Feature #32 — SMPTE Timecode Show (HH:MM:SS:FF @ 24fps)
        // Convert ticks to milliseconds (10,000 ticks per ms)
        const totalMs = Math.floor(Math.abs(ticks) / 10000);
        const fps = 24; // Standard cinema framerate
        ...
        let html = `${hours > 0 ? pad(hours) + ':' : '00:'}${pad(minutes)}:${pad(seconds)}:${pad(frames)}`;
        if (ticks < 0) html = '-' + html;
    ...
    const btnFrameBack = view.querySelector('.btnFrameBack');
    const btnFrameForward = view.querySelector('.btnFrameForward');
    ...
    btnFrameBack.addEventListener('click', function () {
        const player = currentPlayer;
        if (player) {
            if (!player.paused()) playbackManager.pause(player);
            const currentMs = playbackManager.currentTime(player);
            playbackManager.seekMs(currentMs - 42, player);
        }
    });
    btnFrameForward.addEventListener('click', function () {
        const player = currentPlayer;
        if (player) {
            if (!player.paused()) playbackManager.pause(player);
            const currentMs = playbackManager.currentTime(player);
    ...
    if (btnReview) {
        btnReview.addEventListener('click', async function(e) {
            e.stopPropagation();
            chatOverlay.classList.toggle('hide');
            // Lazy load engine to keep initial page speed native
            if (!firebaseEngine && !chatOverlay.classList.contains('hide')) {
                try {
                    chatMessages.innerHTML = "<em style='color:#ccc; display:block; text-align:center; margin-top:15px;'>Mapeando pontos da timeline na nuvem (Firestore)...</em>";
                    firebaseEngine = await import('../../../scripts/capiau/firebaseConfig.js');
                    startFirebaseChatListeners();
                } catch (e) {
                    chatMessages.innerHTML = "<em style='color:#ff3b3b; display:block; text-align:center;'>Erro de comunicação com a Nuvem. Configurações inválidas?</em>";
                    console.error("CapIAu Firebase Failed:", e);
                }
            }
        });
    }
    ...
    if (chatOverlay) {
        ['click', 'dblclick', 'mousedown', 'mouseup', 'pointerdown', 'pointerup', 'touchstart', 'touchend'].forEach(evt => {
            chatOverlay.addEventListener(evt, function(e) { e.stopPropagation(); });
        });
    }

    if (btnChatClose) {
        btnChatClose.addEventListener('click', function(e) {
            e.stopPropagation();
            chatOverlay.classList.add('hide');
        });
    }

    function renderTimelineDots(commentsData) {
        const sliderMarkerContainer = view.querySelector('.sliderMarkerContainer');
        if (!sliderMarkerContainer) return;

        const pState = currentPlayer ? playbackManager.getPlayerState(currentPlayer) : null;
        const nowPlayingItem = pState ? pState.NowPlayingItem : null;
        const runTimeTicks = nowPlayingItem ? nowPlayingItem.RunTimeTicks : 0;
        if (!runTimeTicks) return;
        
        const runTimeMs = runTimeTicks / 10000;
        sliderMarkerContainer.innerHTML = '';
        
        commentsData.forEach(c => {
           const pct = (c.timeMs / runTimeMs) * 100;
           if (pct >= 0 && pct <= 100) {
               ...
               dot.style.width = '8px'; // Aumentado um pouco para facilitar o clique
               dot.style.height = '8px';
               dot.style.backgroundColor = '#ff3b3b';
               dot.style.borderRadius = '50%';
               dot.style.zIndex = '99';
               dot.style.cursor = 'pointer';
               dot.style.pointerEvents = 'auto'; // Força recebimento do hover/click
               
               let titleText = c.comment || '';
               ...
               dot.addEventListener('click', function(e) {
                   ...
                   if (currentPlayer) playbackManager.seekMs(c.timeMs, currentPlayer);
               });
               
               sliderMarkerContainer.appendChild(dot);
           }
        });
    }

    function startFirebaseChatListeners() {
        let capiauUserName = localStorage.getItem('capiauUserName');
        if (!capiauUserName) {
            capiauUserName = prompt("Identifique seu cargo ou nome (ex: Diretor de Arte, Produtor 1):", "Revisor") || "Equipe";
            localStorage.setItem('capiauUserName', capiauUserName);
        }

        // Bloquear os atalhos de teclado do player quando estiver digitando (Space, Arrows, etc)
        ['keydown', 'keyup', 'keypress'].forEach(evt => chatInput.addEventListener(evt, e => e.stopPropagation()));

        const { db, collection, onSnapshot, query, where, addDoc, doc, deleteDoc, updateDoc } = firebaseEngine;
        
        const pState = currentPlayer ? playbackManager.getPlayerState(currentPlayer) : null;
        const nowPlayingItem = pState ? pState.NowPlayingItem : null;
        const mediaId = nowPlayingItem ? nowPlayingItem.Id : 'unknown_item';
        
        if (mediaId === 'unknown_item') return;

        const q = query(collection(db, "capiau_comments"), where("mediaId", "==", mediaId));

        onSnapshot(q, (snapshot) => {
            chatMessages.innerHTML = '';
            
            const docs = snapshot.docs.map(i => ({ id: i.id, ...i.data() })).sort((a,b) => a.timeMs - b.timeMs);
            
            const parents = docs.filter(d => !d.parentId);
            const children = docs.filter(d => d.parentId);
            
            renderTimelineDots(parents); // Dots só para mensagens-pai
            
            if (docs.length === 0) {
                chatMessages.innerHTML = "<em style='color:#666; display:block; text-align:center;'>Timeline limpa. Adicione o seu primeiro comentário.</em>";
                return;
            }
            
            // Renderiza hierarquia Thread
            parents.forEach((pData) => {
                renderChatMessage(pData, false);
                const threads = children.filter(c => c.parentId === pData.id).sort((a,b) => a.timestamp - b.timestamp);
                threads.forEach(cData => renderChatMessage(cData, true));
            });
            chatMessages.scrollTop = chatMessages.scrollHeight;

            function renderChatMessage(data, isChild) {
                const div = document.createElement('div');
                div.style.background = isChild ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.4)";
                div.style.borderLeft = isChild ? "2px solid #555" : "none";
                ...
                const timeStr = new Date(secs * 1000).toISOString().substring(11, 19);
                ...
                const badge = data.user ? `<span style="font-size: 10px; opacity:0.8; background:#333; padding:2px 4px; border-radius:4px; margin: 0 4px;">${data.user}</span>` : '';
                
                const textSpan = document.createElement('span');
                textSpan.innerHTML = `<b style="color:${isChild ? '#999' : '#ff3b3b'};">[${timeStr}]</b>${badge}<span class="comment-text" style="color:#ddd; margin-left: 2px;"></span>`;
                textSpan.querySelector('.comment-text').innerText = data.comment;
                ...
                if (!isChild) {
                    const btnReply = buildIconBtn('reply', '#4facfe', 'Responder');
                    btnReply.addEventListener('click', (e) => {
                        e.stopPropagation();
                        capiauSelectedReplyParent = data.id;
                        chatInput.placeholder = "Respondendo ao ponto " + timeStr + "...";
                        chatInput.focus();
                    });
                    actions.appendChild(btnReply);
                }

                const btnEdit = buildIconBtn('edit', '#a8ff78', 'Editar');
                btnEdit.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const novo = prompt("Editar comentário:", data.comment);
                    if (novo && novo.trim() !== "") {
                        updateDoc(doc(db, "capiau_comments", data.id), { comment: novo.trim() });
                    }
                });
                actions.appendChild(btnEdit);

                const btnDel = buildIconBtn('delete', '#f5576c', 'Apagar');
                btnDel.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if(confirm("Extinguir esta anotação da timeline?")) {
                        deleteDoc(doc(db, "capiau_comments", data.id));
                        // Cascata manual se for pai
                        if (!isChild) {
                            children.filter(c => c.parentId === data.id).forEach(c => deleteDoc(doc(db, "capiau_comments", c.id)));
                        }
                    }
                });
                ...
                div.addEventListener('click', () => {
                    if (currentPlayer) playbackManager.seekMs(data.timeMs, currentPlayer);
                });
                
                chatMessages.appendChild(div);
            }

            function buildIconBtn(icon, color, title) {
            ...
            chatMessages.innerHTML = "<em style='color:#ff3b3b; display:block; text-align:center;'>Erro ao ler Nuvem.</em>";
        });

        // Evento de Inserir Nota ou Resposta
        if (btnChatSend) {
            btnChatSend.addEventListener('click', function() {
                if (!chatInput.value.trim() || !currentPlayer) return;
                
                const currentMs = playbackManager.currentTime(currentPlayer);
                const text = chatInput.value.trim();
                const parentToReply = capiauSelectedReplyParent;
                
                // Clear UI
                chatInput.value = '';
                chatInput.placeholder = "Escreva uma nota...";
                capiauSelectedReplyParent = null; 
                chatInput.focus();
                ...
                if (parentToReply) payload.parentId = parentToReply;

                addDoc(collection(db, "capiau_comments"), payload).catch(e => {
                    console.error("Erro critico de nuvem (Firestore):", e);
                    alert("A anotação não salvou. Verifique Firebase.");
                });
            });
        }
    }
