...
export async function getCommands(options) {
    ...
    // CapIAu-Streaming: Painel Mestre Toggle
    if (['Movie', 'Episode', 'Video'].includes(item.Type) || item.MediaType === 'Video') {
        commands.push({
            name: 'Ativar Painel Mestre (CapIAu)',
            id: 'capiau_master_panel',
            icon: 'fact_check'
        });
    }
    ...
    // CapIAu-Streaming: Feature #8 — Block Downloads
    // Set to true to hide all download options from the UI
    const CAPIAU_BLOCK_DOWNLOADS = true;

    if (!CAPIAU_BLOCK_DOWNLOADS && appHost.supports(AppFeature.FileDownload)) {
...
function executeCommand(item, id, options) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        switch (id) {
            case 'capiau_master_panel':
                window.dispatchEvent(new CustomEvent('capiauToggleStatusPanel', { detail: { mediaId: itemId } }));
